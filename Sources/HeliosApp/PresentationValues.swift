...
struct DisplayValue {
    ...
    init<Value>(_ result: MetricResult<Value>, format: (Value) -> String) {
        switch result {
        case .success(let value): text = format(value); failure = nil
        case .failure(let error): text = "—"; failure = error.localizedDescription
        }
    }
}
...
struct OverviewPresentation {
    ...
    init(_ snapshot: TelemetrySnapshot, now: Date = Date()) {
        ...
        fanOwnershipPreflight = TelemetryFormatting.fresh(snapshot.fanOwnershipPreflight, maxAge: 30, now: now)
    ...
    func temperatures(_ group: ThermalGroup) -> MetricResult<ThermalGroupSummary> {
        thermals.flatMap { metrics in captureMetric { try ThermalGroupSummary.summarize(metrics, group: group) } }
    }
}
