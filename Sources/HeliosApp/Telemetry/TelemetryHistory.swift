...
struct TelemetryHistory: Sendable {
  ...
  mutating func append(_ snapshot: TelemetrySnapshot, now: Date = Date()) {
    ...
    let point = TelemetryHistoryPoint(
      ...
      maxSoCCelsius: value(
        TelemetryFormatting.fresh(snapshot.thermals, maxAge: 6, now: now).flatMap(
          \.maximumSoCCelsius)),
      systemPowerWatts: value(
      ...
      batteryPowerWatts: value(
        TelemetryFormatting.fresh(snapshot.battery, maxAge: 15, now: now).flatMap(\.power).map(
          \.signedWatts)),
      fanRPM: firstFanRPM(snapshot, now: now),
      networkDownloadBytesPerSecond: value(
        TelemetryFormatting.fresh(snapshot.network, maxAge: 5, now: now).flatMap(\.throughput).map(
          \.downloadBytesPerSecond)),
      networkUploadBytesPerSecond: value(
        TelemetryFormatting.fresh(snapshot.network, maxAge: 5, now: now).flatMap(\.throughput).map(
          \.uploadBytesPerSecond)),
      storageReadBytesPerSecond: value(
        TelemetryFormatting.fresh(snapshot.storage, maxAge: 8, now: now).flatMap(\.throughput).map(
          \.readBytesPerSecond)),
      storageWriteBytesPerSecond: value(
        TelemetryFormatting.fresh(snapshot.storage, maxAge: 8, now: now).flatMap(\.throughput).map(
          \.writeBytesPerSecond))
    )
  ...
  private func firstFanRPM(_ snapshot: TelemetrySnapshot, now: Date) -> Double? {
    guard
      case .success(let inventory) = TelemetryFormatting.fresh(snapshot.fans, maxAge: 5, now: now),
