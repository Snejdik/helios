  ...
  static func main() {
      ...
      if CommandLine.arguments.contains("--performance-preflight") {
        Task { @MainActor in
          ...
          case .success(let value):
            ...
            let deviceUsage = TelemetryFormatting.text(
              value.deviceUtilizationPercent, format: { String(format: "%.1f%%", $0) })
            let rendererUsage = TelemetryFormatting.text(
              value.rendererUtilizationPercent, format: { String(format: "%.1f%%", $0) })
            let tilerUsage = TelemetryFormatting.text(
              value.tilerUtilizationPercent, format: { String(format: "%.1f%%", $0) })
            print("  utilization=\(deviceUsage) renderer=\(rendererUsage) tiler=\(tilerUsage)")
            print(
              "  memory mapped="
                + TelemetryFormatting.text(
                  value.allocatedSystemMemoryBytes, format: TelemetryFormatting.storageBytes)
                + " in-use="
                + TelemetryFormatting.text(
                  value.inUseSystemMemoryBytes, format: TelemetryFormatting.storageBytes))
          case .failure(let error):
            print("GPU: unavailable — \(error.localizedDescription)")
          }
          switch p.result.flatMap(\.totalSystemWatts) {
          case .success(let watts):
            print(String(format: "Total System Power (PSTR): %.2f W", watts))
          case .failure(let error):
          ...
          case .success(let value):
            print(
              "Swap: used="
                + TelemetryFormatting.text(
                  value.swapUsedBytes, format: TelemetryFormatting.storageBytes) + " total="
                + TelemetryFormatting.text(
                  value.swapTotalBytes, format: TelemetryFormatting.storageBytes))
          case .failure(let error): print("Memory: unavailable — \(error.localizedDescription)")
          }
          switch b.result {
          case .success(let value):
            let charge = TelemetryFormatting.text(
              value.stateOfChargePercent, format: { String(format: "%.1f%%", $0) })
            let voltage = TelemetryFormatting.text(
              value.voltageVolts, format: { String(format: "%.2f V", $0) })
            let current = TelemetryFormatting.text(
              value.currentAmps, format: { String(format: "%+.2f A", $0) })
            ...
            print(
              "Battery: charge=\(charge) voltage=\(voltage) current=\(current) source=\(source) charging=\(charging) adapter=\(adapter)"
            )
      ...
      if CommandLine.arguments.contains("--utility-preflight") {
        Task { @MainActor in
          ...
          case .success(let value):
            ...
            print(
              "  OS=\(value.osVersion) uptime=\(TelemetryFormatting.duration(value.uptimeSeconds)) CPUs=\(value.logicalProcessorCount) memory=\(TelemetryFormatting.storageBytes(value.physicalMemoryBytes))"
            )
            print(
              "  thermal=\(value.thermalState.rawValue) lowPower=\(value.lowPowerModeEnabled ? "on" : "off")"
            )
            print(
              "  load 1/5/15="
                + TelemetryFormatting.text(value.loadAverage1) { String(format: "%.2f", $0) } + "/"
                + TelemetryFormatting.text(value.loadAverage5) { String(format: "%.2f", $0) } + "/"
                + TelemetryFormatting.text(value.loadAverage15) { String(format: "%.2f", $0) })
          ...
          case .success(let value):
            ...
            print(
              "Network: primary=\(primary) active=\(value.activeInterfaceCount) IPv4=\(ipv4) IPv6=\(ipv6)"
            )
            switch value.throughput {
            case .success(let rate):
              print(
                "  download=\(TelemetryFormatting.bytesPerSecond(rate.downloadBytesPerSecond)) upload=\(TelemetryFormatting.bytesPerSecond(rate.uploadBytesPerSecond)) rx=\(TelemetryFormatting.iops(rate.receivePacketsPerSecond)) pkt/s tx=\(TelemetryFormatting.iops(rate.transmitPacketsPerSecond)) pkt/s"
              )
            case .failure(let error):
              print("  throughput=unavailable — \(error.localizedDescription)")
            }
            print(
              "  link="
                + TelemetryFormatting.text(
                  value.linkSpeedBitsPerSecond, format: TelemetryFormatting.bitsPerSecond) + " MTU="
                + TelemetryFormatting.text(value.mtu) { String($0) } + " RXerr="
                + TelemetryFormatting.text(value.receiveErrors) { String($0) } + " TXerr="
                + TelemetryFormatting.text(value.transmitErrors) { String($0) })
          ...
          case .success(let value):
            print(
              "Battery time remaining: "
                + TelemetryFormatting.text(
                  value.timeRemaining, format: TelemetryFormatting.batteryTimeRemaining))
          case .failure(let error): print("Battery: unavailable — \(error.localizedDescription)")
          }
          print(
            "History/energy integration is deterministic and covered by the regression suite; this probe performs no writes, subprocesses, or privileged telemetry."
          )
          exit(0)
        }
        RunLoop.current.run()
      ...
      if CommandLine.arguments.contains("--observability-preflight") {
        Task { @MainActor in
          ...
          case .success(let value):
            print(
              String(
                format: "CPU: %.1f%% across %d logical cores", value.usagePercent,
                value.perCoreUsagePercent.count))
          ...
          case .success(let value):
            ...
            let rate = TelemetryFormatting.text(value.transmitRateMbps) {
              String(format: "%.0f Mb/s", $0)
            }
            ...
            print(
              "Wi-Fi: interface=\(value.interfaceName) power=\(value.powerOn ? "on" : "off") service=\(value.serviceActive ? "active" : "inactive") SSID=\(ssid)"
            )
            print("  RSSI=\(rssi) noise=\(noise) SNR=\(snr) TX=\(rate) power=\(txPower)")
            print(
              "  channel=\(channel) band=\(band) width=\(width) PHY=\(phy) security=\(security)")
          case .failure(let error):
          ...
          case .success(let value):
            ...
            for process in value.topByCPU.prefix(5) {
              ...
              let power =
                process.powerWatts.map { String(format: "%.3f W direct", $0) } ?? "warming"
              let pPower =
                process.performanceCorePowerWatts.map { String(format: "%.3f W P-core", $0) }
                ?? "warming"
              let ipc = process.instructionsPerCycle.map { String(format: "%.2f", $0) } ?? "—"
              print(
                "  pid=\(process.pid) \(process.name): CPU=\(cpu) energy=\(power) \(pPower) memory=\(TelemetryFormatting.storageBytes(process.physicalFootprintBytes)) ANE=\(TelemetryFormatting.storageBytes(process.neuralFootprintBytes)) IPC=\(ipc)"
              )
            }
          case .failure(let error):
          ...
          print(
            "No fan writes, notification prompts, history writes, subprocesses, or privileged telemetry are used by this probe."
          )
          exit(0)
        }
        RunLoop.current.run()
        ...
        Task { @MainActor in
          ...
          case .success(let value):
            ...
            if let primary = value.primaryDevice, case .success(let counters) = primary.counters {
              print(
                "  physical since boot: read=\(TelemetryFormatting.storageBytes(counters.bytesRead)) written=\(TelemetryFormatting.storageBytes(counters.bytesWritten))"
              )
              if let uptime, uptime > 0 {
                print(
                  "  boot-average: read=\(TelemetryFormatting.bytesPerSecond(Double(counters.bytesRead) / uptime)) written=\(TelemetryFormatting.bytesPerSecond(Double(counters.bytesWritten) / uptime))"
                )
              }
            }
            switch value.throughput {
            case .success(let rate):
              print(
                "  physical current: read=\(TelemetryFormatting.bytesPerSecond(rate.readBytesPerSecond)) write=\(TelemetryFormatting.bytesPerSecond(rate.writeBytesPerSecond))"
              )
            case .failure(let error):
              print("  physical current: unavailable — \(error.localizedDescription)")
            }
            print(
              "  since preflight start: read="
                + TelemetryFormatting.text(
                  value.monitoringReadBytes, format: TelemetryFormatting.storageBytes) + " written="
                + TelemetryFormatting.text(
                  value.monitoringWrittenBytes, format: TelemetryFormatting.storageBytes))
          ...
          case .success(let value):
            print("Processes: accessible=\(value.accessibleProcessCount)")
            print(
              "  accounted current: read=\(TelemetryFormatting.bytesPerSecond(value.accountedDiskReadBytesPerSecond)) write=\(TelemetryFormatting.bytesPerSecond(value.accountedDiskWriteBytesPerSecond))"
            )
            print(
              "  accounted since preflight start: read=\(TelemetryFormatting.storageBytes(value.sessionAccountedReadBytes)) written=\(TelemetryFormatting.storageBytes(value.sessionAccountedWriteBytes))"
            )
            if let reader = value.topByDiskRead.first, let rate = reader.diskReadBytesPerSecond {
              print(
                "  top reader: pid=\(reader.pid) \(reader.name) \(TelemetryFormatting.bytesPerSecond(rate))"
              )
            }
            if let writer = value.topByDiskWrite.first, let rate = writer.diskWriteBytesPerSecond {
              print(
                "  top writer: pid=\(writer.pid) \(writer.name) \(TelemetryFormatting.bytesPerSecond(rate))"
              )
            }
            if let helios = value.heliosActivity {
              let read =
                helios.diskReadBytesPerSecond.map(TelemetryFormatting.bytesPerSecond) ?? "warming"
              let write =
                helios.diskWriteBytesPerSecond.map(TelemetryFormatting.bytesPerSecond) ?? "warming"
              print(
                "  Helios itself: CPU=\(helios.cpuPercent.map { String(format: "%.2f%%", $0) } ?? "warming") read=\(read) write=\(write) memory=\(TelemetryFormatting.storageBytes(helios.physicalFootprintBytes))"
              )
            }
          ...
          case .success(let value):
            print(
              "Network session: down="
                + TelemetryFormatting.text(
                  value.sessionDownloadedBytes, format: TelemetryFormatting.storageBytes) + " up="
                + TelemetryFormatting.text(
                  value.sessionUploadedBytes, format: TelemetryFormatting.storageBytes))
          case .failure(let error): print("Network: unavailable — \(error.localizedDescription)")
          }

          print(
            "Physical storage counters include all macOS/APFS/VM/cache/kernel I/O. libproc process accounting is a different layer and is not expected to sum exactly to physical device bytes."
          )
          print(
            "No fan writes, persistent history/audit writes, subprocesses, notification prompts, or privileged telemetry are used by this probe."
          )
          exit(0)
        }
        RunLoop.current.run()
      ...
      if CommandLine.arguments.contains("--feature-preflight") {
        Task { @MainActor in
          ...
          case .success(let value):
            print(
              String(
                format: "CPU: %.1f%% logical=%d physical=%@ P=%@ E=%@",
          ...
          case .success(let value):
            print(
              "Memory: used=\(TelemetryFormatting.storageBytes(value.usedBytes)) app=\(TelemetryFormatting.storageBytes(value.appBytes)) cache=\(TelemetryFormatting.storageBytes(value.cacheBytes)) available=\(TelemetryFormatting.storageBytes(value.availableBytes))"
            )
            print(
              "  pressure=" + TelemetryFormatting.text(value.pressure) { $0.rawValue } + " swap="
                + TelemetryFormatting.text(
                  value.swapUsedBytes, format: TelemetryFormatting.storageBytes)
                + " in/out=\(value.swapIns)/\(value.swapOuts)")
          case .failure(let error): print("Memory: unavailable — \(error.localizedDescription)")
          }
          switch b.result {
          case .success(let value):
            let systemSoC = TelemetryFormatting.text(value.systemChargePercent) {
              String(format: "%.1f%%", $0)
            }
            let rawSoC = TelemetryFormatting.text(value.rawStateOfChargePercent) {
              String(format: "%.1f%%", $0)
            }
            let health = TelemetryFormatting.text(value.healthPercent) {
              String(format: "%.1f%%", $0)
            }
            let optimized = TelemetryFormatting.text(value.optimizedChargingEngaged) {
              $0 ? "engaged" : "not engaged"
            }
            print(
              "Battery: systemSoC=\(systemSoC) rawSoC=\(rawSoC) health=\(health) optimized=\(optimized)"
            )
            print(
              "  raw current=" + TelemetryFormatting.text(value.currentCapacityMAh) { "\($0) mAh" }
          ...
          case .success(let value):
            ...
            for item in value.displays.prefix(4) {
              ...
              print(
                "  \(item.label): \(item.pixelWidth)x\(item.pixelHeight) px / \(item.logicalWidth)x\(item.logicalHeight) logical\(rate)"
              )
            }
          ...
          case .success(let value):
            print(
              "Mounted volumes: \(value.volumes.count) (external/removable=\(value.volumes.filter { $0.isRemovable == true || $0.isInternal == false }.count))"
            )
          case .failure(let error):
            print("Mounted volumes: unavailable — \(error.localizedDescription)")
          }
          switch u.result {
          ...
          case .success(let value):
            print(
              "Bluetooth: paired=\(value.devices.count) connected=\(value.devices.filter(\.connected).count) (no active scan)"
            )
          ...
          case .success(let value):
            let inputSummary =
              value.inputTelemetrySuppressedForPrivacy
              ? "not probed (privacy-safe)" : (value.defaultInput?.name ?? "—")
            print(
              "Audio: devices=\(value.devices.count) input=\(inputSummary) output=\(value.defaultOutput?.name ?? "—")"
            )
          ...
          case .success(let value):
            print(
              "Sleep blockers: assertions=\(value.assertions.count) display=\(value.displaySleepBlockers.count) system=\(value.systemSleepBlockers.count)"
            )
          case .failure(let error):
            print("Sleep blockers: unavailable — \(error.localizedDescription)")
          }
          if case .success(let value) = cl.result {
            print(
              "Clock: zone=\(value.localTimeZoneIdentifier) ISO-week=\(value.isoWeekOfYear) day-of-year=\(value.dayOfYear)"
            )
          }
          print(
            "Battery telemetry is read-only; charging policy remains owned by macOS and Helios exposes no battery-control write path."
          )
          print(
            "No fan writes, persistent writes, subprocesses, active Bluetooth scans, notification prompts, or privileged telemetry are used by this probe."
          )
          exit(0)
        }
        RunLoop.current.run()
      ...
      if CommandLine.arguments.contains("--smc-inventory-preflight") {
        Task { @MainActor in
          ...
          case .success(let value):
            print(
              "Numeric channels: \(value.readings.count)\(value.truncated ? " (truncated)" : "") failures=\(value.failures.count)"
            )
            for reading in value.readings.prefix(40) {
              print(
                String(format: "  %@ type='%@' raw=%.6g", reading.key, reading.type, reading.value))
            }
          case .failure(let error):
            print("SMC numeric inventory unavailable — \(error.localizedDescription)")
          }
          print(
            "Values are intentionally unitless unless a dedicated Helios provider has validated their meaning. Unknown keys never enter fan safety."
          )
          print(
            "No fan writes, battery-control writes, persistent writes, subprocesses, notification prompts, or privileged telemetry are used by this probe."
          )
          exit(0)
        }
        RunLoop.current.run()
      ...
      if CommandLine.arguments.contains("--maintenance-preflight") {
        Task { @MainActor in
          ...
          if case .success(let value) = c.result {
            print(
              "Cleanup scout: \(TelemetryFormatting.storageBytes(value.estimatedBytes)) across \(value.candidates.count) known cache/developer locations"
            )
            for item in value.candidates {
              print(
                "  \(item.label): \(TelemetryFormatting.storageBytes(item.estimatedBytes)) entries=\(item.scannedEntries)\(item.truncated ? " truncated" : "")"
              )
            }
          }
          if case .success(let value) = a.result {
            ...
            let measured = value.applications.compactMap(\.estimatedSizeBytes).reduce(UInt64(0)) {
              lhs, rhs in
            ...
            print(
              "Applications: \(value.applications.count) AppleSilicon=\(arm) Universal=\(universal) Intel=\(intel) observedSize=\(TelemetryFormatting.storageBytes(measured)) incompleteSizeScans=\(incomplete)"
            )
            for app in value.applications.sorted(by: {
              ($0.estimatedSizeBytes ?? 0) > ($1.estimatedSizeBytes ?? 0)
            }).prefix(8) {
              let size =
                app.estimatedSizeBytes.map(TelemetryFormatting.storageBytes) ?? "unmeasured"
              print(
                "  \(app.name): \(size) · \(app.architecture.rawValue)\(app.sizeTruncated ? " · partial" : "")"
              )
            }
          }
          print(
            "This on-demand probe may read application bundles/cache metadata, but never deletes or modifies anything and launches no subprocesses."
          )
          exit(0)
        }
        RunLoop.current.run()
      ...
      if CommandLine.arguments.contains("--unregister-helper")
        || CommandLine.arguments.contains("--reinstall-helper")
      {
        Task { @MainActor in
          exit(
            await DaemonRegistrationProbe.manage(
              reinstall: CommandLine.arguments.contains("--reinstall-helper")))
        }
        RunLoop.current.run()
        return
      }
