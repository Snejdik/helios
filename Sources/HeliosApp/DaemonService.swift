...
final class NativeServiceRegistration: ServiceRegistrationDriver {
    private var service: SMAppService { SMAppService.daemon(plistName: HeliosServiceIdentity.launchDaemonPlistName) }
...
final class DaemonService: NSObject, ObservableObject {
    ...
    private let logger = Logger(subsystem: HeliosServiceIdentity.appIdentifier, category: "Registration")
    private var monitor: Task<Void, Never>?
    private var sleeping = false

    init(driver: any ServiceRegistrationDriver = NativeServiceRegistration(), connectAutomatically: Bool = true) {
        self.driver = driver
    ...
    func start() {
        ...
        NSWorkspace.shared.notificationCenter.addObserver(self, selector: #selector(refreshAfterActivation), name: NSWorkspace.didActivateApplicationNotification, object: nil)
    ...
    func refresh() {
        ...
        if state == .installed && !sleeping && connectAutomatically { client.connect() }
        else if client.state != .disconnected { client.disconnect() }
    }
    ...
    func install() {
        ...
        catch { report(error, operation: "Installation") }
        busy = false
        refresh()
    }
    ...
    func uninstall() async {
        ...
        clearError()
        client.disconnect()
        do { try await unregisterAndWait() }
        catch { report(error, operation: "Removal") }
        busy = false
        refresh()
    }
    ...
    func reinstall() async {
        ...
        do {
            ...
            if driver.status == .enabled || driver.status == .requiresApproval { try await unregisterAndWait() }
            try driver.register()
    ...
    private func unregisterAndWait() async throws {
        ...
        while true {
            ...
            } else { missingSince = nil }
            guard ContinuousClock.now < deadline else {
                throw NSError(domain: "com.snejda.Helios.Registration", code: 1,
                              userInfo: [NSLocalizedDescriptionKey: "Service removal has not settled; replacement registration was not attempted."])
            }
    ...
    private func clearError() { message = nil; errorDetail = nil }
    ...
    private func report(_ error: Error, operation: String) {
        ...
        if error.code == kSMErrorInvalidSignature,
           error.domain == "SMAppServiceErrorDomain" {
            message = "macOS rejected this build's signature. Configure Apple Development signing and rebuild."
        } else if driver.status == .requiresApproval {
            message = "Approve Helios in System Settings → Login Items & Extensions."
        } else {
            message = "\(operation) could not be completed. Check this build's signing and helper registration."
        }
        logger.error("\(operation, privacy: .public): \(self.errorDetail ?? "Unknown failure", privacy: .public)")
    }
}
...
enum DaemonRegistrationProbe {
    ...
    static func manage(reinstall: Bool) async -> Int32 {
        ...
        if let error = service.errorDetail { print(error); return 1 }
    ...
    static func verifyConnection() async -> Int32 {
        ...
        guard client.state == .connected, let version = client.negotiatedVersion,
              let pid = client.peerPID, let uid = client.peerUID, uid == 0, pid != getpid() else {
        ...
        while start.duration(to: .now) < .seconds(12) {
            ...
            guard client.state == .connected, client.peerPID == pid else {
                print("FAIL: \(client.detail ?? "Connection lost")"); return 1
            }
        ...
        guard observed >= 10 else { print("FAIL: Fewer than ten completed heartbeats"); return 1 }
        print("PASS: Installed, authenticated cross-process handshake and \(observed) heartbeats over 12 seconds")
        return 0
    }
    ...
    static func run() async -> Int32 {
        ...
        do { try driver.register(); print("register(): succeeded") }
        catch {
            let error = error as NSError
        ...
        if after == .enabled || after == .requiresApproval {
            do { try await driver.unregister(); print("unregister(): succeeded") }
            catch { print("Cleanup failed: \(error)"); return 1 }
        }
